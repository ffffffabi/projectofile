    ...
    $.ajax({
     ...
     success:function(data)
     {
        ...
        $this.parent().find('.alert-danger').show();
         $this.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
             $this.parent().find('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>');
          }
if(admin_loader == 1)
  {
    $('.submit-loader').hide(); 
...
$(document).on('click','#user-notf',function(){
